...
class hardware:
                ...
                'power': {
                    ...
                    'settings': {
                        ...
                        'blueled': {
                            ...
                            'action': 'set_blueled',
                            'type': 'multivalue',
                            'values': ['none', 'heartbeat', 'default-on', 'panic', 'emmc', 'sd'],
                            },
                        'remote_power': {
                            'order': 3,
                            'name': 32416,
    ...
    def start_service(self):
        try:
            ...
            if not 'hidden' in self.struct['fan']:
                self.initialize_fan()
            if not 'hidden' in self.struct['power']['settings']['blueled']:
                self.set_blueled()
    ...
    def load_values(self):
        try:
            ...
            if not os.path.exists('/sys/firmware/devicetree/base/leds/blueled'):
                self.struct['power']['settings']['blueled']['hidden'] = 'true'
            else:
                if 'hidden' in self.struct['power']['settings']['blueled']:
                    del self.struct['power']['settings']['blueled']['hidden']
                value = self.oe.read_setting('hardware', 'blueled')
                if not value is None:
                    self.struct['power']['settings']['blueled']['value'] = value
    ...
    def set_blueled(self, listItem=None):
        try:
            self.oe.dbg_log('hardware::set_blueled', 'enter_function', 0)
            self.oe.set_busy(1)
            if not listItem == None:
                self.set_value(listItem)
            if os.access('/sys/devices/platform/leds/leds/blue:heartbeat/trigger', os.W_OK):
                if not self.struct['power']['settings']['blueled']['value'] is None and not self.struct['power']['settings']['blueled']['value'] == '':
                    blueled_ctl = open('/sys/devices/platform/leds/leds/blue:heartbeat/trigger', 'w')
                    blueled_ctl.write(self.struct['power']['settings']['blueled']['value'])
                    blueled_ctl.close()
            self.oe.dbg_log('hardware::set_blueled', 'exit_function', 0)
        except Exception, e:
            self.oe.dbg_log('hardware::set_blueled', 'ERROR: (%s)' % repr(e), 4)
        finally:
            self.oe.set_busy(0)
